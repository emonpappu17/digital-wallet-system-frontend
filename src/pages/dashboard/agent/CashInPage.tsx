...
import { useCashInMutation } from "@/redux/features/transactionApi";
...
const CashInPage = () => {
    ...
    const cashInData = getCashInData?.data;
    ...
    const validateAmount = (value: string) => {
        ...
        if (numValue < 10) return 'Minimum cash in amount is ৳10';
        if (numValue > 50000) return 'Maximum cash in amount is ৳50,000';
    ...
    const handleSearchUser = async () => {
        if (!searchTerm || searchTerm === '') return toast.error('Please enter user account number');
        try {
    ...
    const commission = (Number(amount) * FEE_CONFIG.agentCommissionPercent) / 100;

    const remainingBalance = commission + (Number(walletBalance) - Number(amount));
            ...
            <CardContent className="space-y-4">
                ...
                <Alert>
                    ...
                    <AlertDescription className="text-sm">
                        Cash in limit: ৳10 - ৳50,000 per transaction
                    </AlertDescription>
                </Alert>
        ...
        return <Card className="w-full max-w-[500px] mx-auto bg-card/45">
            ...
            <CardContent className="space-y-4 ">
                ...
                <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription className="text-sm">
                        Only cash in who has PayWave user account
                    </AlertDescription>
                </Alert>
        ...
        return <Card className="w-full max-w-[500px] mx-auto bg-card/45">
            ...
            <CardContent className="space-y-4 ">
                <div className="bg-gray-50 dark:bg-gray-800 rounded-md p-3 space-y-2">
                    ...
                    <div className="flex justify-between text-sm text-muted-foreground">
                        <div>User Receive</div>
                        <div className="font-medium">৳{amount}</div>
                    </div>
                    <Separator />
                    <div className="flex justify-between font-semibold text-lg">
                        <div>Total</div>
                        <div>৳{(Number(amount) + commission)}</div>
                    </div>
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-md p-3 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm text-muted-foreground">{userData?.phoneNumber} • {userData?.email}</div>
                    </div>
                </div>
                ...
                <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription className="text-sm">
                        After confirm money will be added to user account
                    </AlertDescription>
                </Alert>
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={handleInOut} disabled={cashInLoading} className=" text-white flex-1"
                    >
                        {cashInLoading ? (
                            <>
                                <LoadingSpinner size="sm" className="mr-2 " />
        ...
        return <Card className="w-full max-w-md mx-auto bg-card/45">
            <CardContent className="pt-6">
                <div className="text-center space-y-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-green-600">Cash in successful</h2>
                        <p className="text-sm text-muted-foreground">Your Cash in has been completed successfully</p>
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-md p-3 text-left">
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                <span>Time</span>
                                <span className="font-mono">{handleFormateDate(cashInData?.createdAt)}</span>
                            </div>
                            <div className="flex justify-between">
                                <span>Transaction ID</span>
                                <span className="font-mono">{cashInData?.transactionId}</span>
                            </div>
                            <div className="flex justify-between">
