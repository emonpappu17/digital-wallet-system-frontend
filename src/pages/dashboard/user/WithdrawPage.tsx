...
import { AlertCircle, ArrowLeft, CheckCircle, MapPin, Phone, RefreshCw, Search, Star, User, Wallet } from 'lucide-react';
...
const WithdrawMoneyPage = () => {
  ...
  // Filter agents based on search and cash availability
  const filteredAgents = mockAgents.filter(agent => {
    const matchesSearch = agent.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      agent.location.toLowerCase().includes(searchTerm.toLowerCase());
    const hasEnoughCash = parseFloat(amount) ? agent.cashAvailable >= parseFloat(amount) : true;
    return matchesSearch && hasEnoughCash && agent.isOnline;
  });
  ...
  const validateAmount = (value) => {
    ...
    if (numValue < 50) return 'Minimum withdrawal amount is ৳50';
    if (numValue > mockUser.balance) return 'Insufficient balance';
    if (numValue > 25000) return 'Maximum withdrawal amount is ৳25,000 per transaction';
    return null;
  };
  ...
  const renderAmountStep = () => {
    ...
    const totalDeduction = amount ? parseFloat(amount) + fee : 0;

    return (
      <Card className="w-full max-w-md mx-auto ">
        <CardHeader>
          <CardTitle className="text-center">Withdraw Money</CardTitle>
        ...
        <CardContent className="space-y-4">
          <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            ...
            <p className="text-2xl font-bold text-blue-600">৳{mockUser.balance.toLocaleString()}</p>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[500, 1000, 2000, 5000].map((quickAmount) => (
              <Button
                ...
                className="text-xs"
                disabled={quickAmount > mockUser.balance}
              >
                ৳{quickAmount}
              </Button>
            ))}
          ...
          {amount && (
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 space-y-2">
              ...
              <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>৳{(mockUser.balance - totalDeduction).toLocaleString()}</span>
              </div>
            </div>
          )}
          ...
          <Alert>
            ...
            <AlertDescription className="text-sm">
              Withdrawal limits: ৳50 - ৳25,000 per transaction. Service fees apply.
            </AlertDescription>
          </Alert>

          <Button onClick={handleNextStep} className="w-full" disabled={!amount}>
            Continue
          </Button>
        </CardContent>
      </Card>
      ...
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardDescription>
              Choose an agent to withdraw ৳{amount}
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-green-600">
                        ...
                        Cash Available: ৳{agent.cashAvailable.toLocaleString()}
                      </div>
                    </div>
                  </div>
        ...
        {filteredAgents.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            ...
            <p>No agents found with sufficient cash for this withdrawal</p>
            <p className="text-sm mt-2">Try reducing the amount or search in different area</p>
          </div>
        )}

        <Button onClick={handleNextStep} className="w-full" disabled={!selectedAgent}>
      ...
      <Card className="w-full max-w-md mx-auto">
        ...
        <CardContent className="space-y-4">
          ...
          <Alert>
            <AlertCircle className="h-4 w-4" />
            <AlertDescription className="text-sm">
              You will receive ৳{parseFloat(amount).toLocaleString()} in cash from the agent. Please verify the amount before confirming.
            </AlertDescription>
          </Alert>

          <Button onClick={processWithdrawal} className="w-full" disabled={isLoading}>
            {isLoading ? (
              <>
      ...
      <Card className="w-full max-w-md mx-auto">
        <CardContent className="pt-6">
          <div className="text-center space-y-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-green-600">Withdrawal Request Approved!</h2>
            ...
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription className="text-sm">
                Visit the agent to collect your ৳{parseFloat(amount).toLocaleString()} in cash. Show this transaction ID for verification.
              </AlertDescription>
            </Alert>
